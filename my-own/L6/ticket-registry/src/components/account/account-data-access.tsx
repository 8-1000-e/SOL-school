...
import { type Address, airdropFactory, createTransaction, getBase58Decoder, lamports, signAndSendTransactionMessageWithSigners, type SolanaClient } from 'gill'
...
async function getTokenAccountsByOwner(rpc: SolanaClient['rpc'], { address, programId }: { address: Address; programId: Address }) {
  return await rpc.getTokenAccountsByOwner(address, { programId }, { commitment: 'confirmed', encoding: 'jsonParsed' }).send().then((res) => res.value ?? [])
}
...
export function useTransferSolMutation({ address }: { address: Address }) {
  ...
  return useMutation({
    mutationFn: async (input: { destination: Address; amount: number }) => {
      try {
        ...
        const transaction = createTransaction({
          feePayer: signer, version: 0, latestBlockhash,
          instructions: [getTransferSolInstruction({ amount: input.amount, destination: input.destination, source: signer })],
        })
    ...
    onSuccess: async (tx) => { toastTx(tx); await Promise.all([invalidateBalanceQuery(), invalidateSignaturesQuery()]) },
...
export function useRequestAirdropMutation({ address }: { address: Address }) {
  ...
  return useMutation({
    mutationFn: async (amount: number = 1) =>
      airdrop({ commitment: 'confirmed', recipientAddress: address, lamports: lamports(BigInt(Math.round(amount * 1_000_000_000))) }),
    onSuccess: async (tx) => { toastTx(tx); await Promise.all([invalidateBalanceQuery(), invalidateSignaturesQuery()]) },
  })
}
